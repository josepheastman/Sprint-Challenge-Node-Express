...
router.get('/actions/:projectId', (req, res) => {
    ...
    .catch(err => {
        res.status(500)
        .json({ error: "The projects action information could not be retrieved." })
    })
})
...
//     }
//     projectDb
//     .insert(projectInfo.name, projectInfo.description)
//     .then(result => {
//         projectDb
//         .get(result.id)
//         .then(project => {
...
router.put("/:projectId", (req, res) => {
    ...
    .then(post => {
        if (!post) {
            ...
            projectDb.update(projectId, {name, description, completed})
            .then(count => {
